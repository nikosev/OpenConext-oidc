...
public class JpaOAuth2ClientRepository implements OAuth2ClientRepository {
	...
	public Collection<ClientDetailsEntity> getClientsByUserId(String userId) {
		TypedQuery<ClientDetailsEntity> query = manager.createNamedQuery(ClientDetailsEntity.QUERY_BY_USER_ID, ClientDetailsEntity.class);
		query.setParameter(ClientDetailsEntity.PARAM_USER_ID, userId);
