...
public class DefaultSystemScopeService implements SystemScopeService {
	...
	private Predicate<SystemScope> isScope = new Predicate<SystemScope>() {
		@Override
		public boolean apply(SystemScope input) {
			return (input != null && getAll().contains(input));
		}
	};
	...
	@Override
	public Set<SystemScope> getNoRestrictedDefaults() {
		return Sets.filter(getAll(), Predicates.and(isDefault,Predicates.not(isRestricted)));
	}
	...
	@Override
	public Set<SystemScope> removeUndefinedScopes(Set<SystemScope> scopes) {
		return Sets.filter(scopes, isScope);
	}
